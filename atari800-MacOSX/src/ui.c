...
static void DiskManagement(void)
{
	...
	for (;;) {
		...
		case 11:
#ifdef MACOSX
			if (UI_driver->fGetSaveFilename(disk_filename, atari_disk_dirs, 1)) {
#else				
		...
		case 12:
#ifdef MACOSX					
			if (UI_driver->fGetLoadFilename(disk_filename, atari_disk_dirs, 1)) {
#else
			if (UI_driver->fGetLoadFilename(disk_filename, UI_atari_files_dir, UI_n_atari_files_dir)) {
				...
				case 0x1f:
					fclose(fp);
#ifdef MACOSX
					if (UI_driver->fGetSaveFilename(uncompr_filename, atari_disk_dirs, 1)) {
#else
		...
		case 2:
			...
			if (UI_driver->fGetLoadFilename(cart_filename, UI_atari_files_dir, UI_n_atari_files_dir)) {
				...
				if (CARTRIDGE_main.type != CARTRIDGE_NONE) {
					/* Auto machine type switching handled elsewhere */
				}
				Atari800_Coldstart();
				return;
			}
...
static void SoundRecording(void)
{
	...
	{	filename = SndSave_Find_AIFF_name();
		if (SndSave_OpenSoundFile(filename))
			sprintf(msg, "Recording sound to file \"%s\"",filename);
		else
			sprintf(msg, "Can't write to file \"%s\"",filename);
	}
	else
...
/* D_PATCH not available in this version */
		UI_MENU_CHECK(6, "P: device (printer):"),
#ifdef R_IO_DEVICE
		UI_MENU_CHECK(7, "R: device (Atari850 via net):"),
#endif
#ifndef MACOSX		
		UI_MENU_FILESEL_PREFIX(8, "H1: ", Devices_atari_h_dir[0]),
		UI_MENU_FILESEL_PREFIX(9, "H2: ", Devices_atari_h_dir[1]),
		UI_MENU_FILESEL_PREFIX(10, "H3: ", Devices_atari_h_dir[2]),
		UI_MENU_FILESEL_PREFIX(11, "H4: ", Devices_atari_h_dir[3]),
		UI_MENU_SUBMENU(21, "Advanced H: options"),
		UI_MENU_ACTION_PREFIX(12, "Print command: ", Devices_print_command),
		UI_MENU_FILESEL_PREFIX(13, " OS/A ROM: ", CFG_osa_filename),
		UI_MENU_FILESEL_PREFIX(14, " OS/B ROM: ", CFG_osb_filename),
		UI_MENU_FILESEL_PREFIX(15, "XL/XE ROM: ", CFG_xlxe_filename),
		UI_MENU_FILESEL_PREFIX(16, " 5200 ROM: ", CFG_5200_filename),
		UI_MENU_FILESEL_PREFIX(17, "BASIC ROM: ", CFG_basic_filename),
		UI_MENU_FILESEL(18, "Find ROM images in a directory"),
		UI_MENU_SUBMENU(19, "Configure directories"),
		UI_MENU_ACTION(20, "Save configuration file"),
#endif		
		UI_MENU_END
	};
#ifndef MACOSX	
...
#ifdef MACOSX		
		menu_array[3].suffix = Devices_enable_h_patch ? (Devices_h_read_only ? "Read-Only" : "Read/write") : "No ";
#else		
		menu_array[4].suffix = Devices_enable_h_patch ? (Devices_h_read_only ? "Read-Only" : "Read/write") : "No ";
#endif		
/* D_PATCH not available */
		SetItemChecked(menu_array, 6, Devices_enable_p_patch);
#ifdef R_IO_DEVICE
		SetItemChecked(menu_array, 7, Devices_enable_r_patch);
#endif
...
#ifdef MACOSX
static char config_filename[FILENAME_MAX] = "";
#endif
...
#ifdef XEP80_EMULATION
		/* XEP80 not available in this version */
#endif
...
#endif
/* SERIO_SOUND not available in this version */
...
#ifdef MACOSX
	if (UI_driver->fGetSaveFilename(filename, atari_image_dir, 1)) {
#else
...
#define MENU_CONTROLLER 19
...
#endif
#if !defined(USE_CURSES) && !defined(MACOSX)
...
#elif defined(MACOSX)		
		UI_MENU_FILESEL_ACCEL(UI_MENU_SAVECFG, "Save Configuration", "Alt+Shift+S"),
		UI_MENU_FILESEL_ACCEL(UI_MENU_LOADCFG, "Load Configuration", "Alt+Shift+L"),
		UI_MENU_FILESEL_ACCEL(UI_MENU_PCX, "TIFF Screenshot", "F13"),
#else
	...
	while (!done) {
		...
		case UI_MENU_LOADSTATE:
			...
			break;
#ifdef MACOSX
		case UI_MENU_SAVECFG:
			SaveConfig();
			break;
		case UI_MENU_LOADCFG:
			LoadConfig();
			break;
#endif
#ifndef CURSES_BASIC
