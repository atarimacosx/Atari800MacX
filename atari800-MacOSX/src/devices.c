...
#ifdef D_PATCH	
	/* If this is called from the D: handler, not the H: */
	if (in_d_patch) {
		/* It must be the default drive, as we normally don't 
		   handle requests with device number of 1 */
		if (devnum == 1) {
			devnum = d_current_drive;
		}
		...
		/* D5: and D6: don't do translation, D7: and D8: do */
		if (devnum < 7) {
			if (set_textmode)
...
#ifdef D_PATCH	
#define D_DEVICE_BEGIN  0xd1c0
#define D_TABLE_ADDRESS 0xd1c0
#define D_PATCH_OPEN    0xd1d0
#define D_PATCH_CLOS    0xd1d3
#define D_PATCH_READ    0xd1d6
#define D_PATCH_WRIT    0xd1d9
#define D_PATCH_STAT    0xd1dc
#define D_PATCH_SPEC    0xd1df
#define D_PATCH_INIT    0xd1e3
#define D_DEVICE_END    0xd1e5
#endif
