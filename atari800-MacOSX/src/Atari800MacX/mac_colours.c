...
void Colours_Generate(int black, int white, int colors, int shift)    /* generate a fresh palette */
    {
      int i, j;
      pal_t rgb;
      ...
      for (i = 0; i < 0x10; i++)
	{
      ...
      for (i = 0; i < 0x100; i++)
      {
	  basicpal[i] = (rgb[i].r << 16) + (rgb[i].g << 8) + (rgb[i].b << 0);
      }
    }
...
void Colours_Initialise(int *argc, char *argv[])
{
	...
	for (i = j = 1; i < *argc; i++)
	{
		if (strcmp(argv[i], "-black") == 0)
			...
			read_palette(argv[++i], FALSE);
		else {
			if (strcmp(argv[i], "-help") == 0) {
				Log_print("\t-palette <file>  Use external palette");
				Log_print("\t-black <0-255>   Set black level");
				Log_print("\t-white <0-255>   Set white level");
				Log_print("\t-colors <num>    Set color intensity");
				Log_print("\t-genpal <num>    Generate artificial palette");
				Log_print("\t-colshift <num>  Set color shift (-genpal only)");
			}
			argv[j++] = argv[i];
		}
	}
	*argc = j;
	...
	if (generate_palette)
                Colours_Generate(min_y, max_y, colintens, colshift);
	if (! palette_loaded)
		Colours_Format(min_y, max_y, colintens);
}
...
int read_palette(char *filename, int adjust_palette) {
	...
	palette_loaded = TRUE;
        if (adjust_palette)
            memcpy(basicpal, colortable, sizeof(basicpal));
	return TRUE;
}
...
palette adjusting code improved and moved to Colours_Format function
